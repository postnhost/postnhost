...
CRITICAL: Do not add fallbacks or backward-compatibility code paths. This project is pre-release, so prefer intentional breaking changes that reduce code and improve quality.
...
Never rely on host-application files, parent instructions, or parent-relative runtime paths. The sibling `postnhost-app` repository may consume this engine locally, but the engine must remain independently buildable and testable.
