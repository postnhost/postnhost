...
RSpec.describe Postnhost::InlineArticleImageUploader do
  around do |example|
    with_isolated_uploader_root { example.run }
  end

  describe "#store!" do
    it "stores a generated webp inline image under the article directory" do
      article = create(:article)
      uploader = described_class.new(article, :inline_image)

      allow(SecureRandom).to receive(:hex).with(12).and_return("a" * 24)

      uploader.store!(uploaded_image(postnhost_asset_image_path("demo.webp"), "image/webp"))

      expect(uploader.store_dir).to eq("uploads/articles/#{article.user_id}/#{article.id}/inline_images")
      expect(uploader.identifier).to eq("#{'a' * 24}.webp")
